...
| v0.7 | Scaling to real-application MC/DC (per `v07-scaling-roadmap.md`). Destination workload: `seanmonstar/httparse` (~1500 decisions). Streaming counter encoding, i64 saturating counters, inlined-subroutine DWARF, module-rollup reports, BrTable per-row reconstruction. | DWARF parsing memory bound at scale. |
