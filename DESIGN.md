...
| v0.5 | **Workspace split**: `witness-core` library compiles to `wasm32-wasip2`; `witness` binary keeps the wasmtime runner. **`witness lcov`** subcommand emits LCOV (DWARF-correlated `BRDA` + sibling overview) for codecov ingestion. **CI dogfood**: `witness measures the fixture` step uploads LCOV to codecov with flag `wasm-bytecode` alongside `rust-source`. **Sigil/wsc integration**: `witness attest` produces DSSE-signed envelopes via `wsc-attestation::dsse::DsseEnvelope::sign_ed25519`. **Wasm artefact**: `witness-core` Wasm build attached to releases. Loom + meld upstream issue drafts in `docs/research/v05-loom-meld-upstream.md`. | Resolved: workspace, LCOV, attest, wasm artefact all shipped. The CHANGELOG described coverage as "MC/DC" but the report layer computed only branch coverage; v0.6 redo addresses this. |
| v0.6 | **Redo of the MC/DC claim.** Schema additions for per-decision row tables (`RunRecord.decisions`, `DecisionRecord`, `DecisionRow`, `TraceHealth`). New `mcdc_report` module emits truth tables, independent-effect citations under masking MC/DC (DO-178C accepted), gap-closure recommendations with row-flip vectors. New `witness report --format mcdc` / `--format mcdc-json` CLI. Verdict suite (`verdicts/`) of seven canonical compound-decision shapes (leap_year, range_overlap, triangle, state_guard, mixed_or_and, safety_envelope, parser_dispatch) with hand-derived `TRUTH-TABLE.md` oracles and per-verdict `V-MODEL.md` traceability. Four parallel research agents shipped roadmap docs for v0.7 (scaling to httparse), v0.8 (Axum+HTMX+wstd-axum visualisation with MCP server), v0.9 (state-of-the-art + AI-agent UX positioning). v0.6.0 ships the consumer side (schema, reporter, oracles, CLI) and **defers the on-Wasm trace-buffer instrumentation rewrite to v0.6.1**, per the explicit recommendation in `docs/research/v06-instrumentation-primitive.md`. The original v0.6 plan (loom/meld DWARF + component-model coverage) shifts to v0.7+. | Resolved on consumer side; v0.6.1 closes the runtime instrumentation. |
| v0.6.1 | On-Wasm trace-buffer instrumentation per `docs/research/v06-instrumentation-primitive.md`: linear-memory trace buffer with row markers, exported `__witness_row_marker(u32)`, runner-side row demarcation, `RunRecord.decisions` populated from execution rather than hand-curated. Verdict suite end-to-end execution; `compliance` action ships real per-verdict signed predicates. | None — research brief gates implementation only. |
| v0.7 | Scaling to real-application MC/DC (per `v07-scaling-roadmap.md`). Destination workload: `seanmonstar/httparse` (~1500 decisions). Streaming counter encoding, i64 saturating counters, inlined-subroutine DWARF, module-rollup reports, BrTable per-row reconstruction. **Reconciled 2026-06:** i64 counters shipped v0.32 (FEAT-036; *not* saturation-guarded — descoped, DEC-039). BrTable per-row reconstruction shipped v0.33 (FEAT-038, via the bit-level audit; also fixed a soundness bug where arms were "proved" by mere execution). **Streaming counter encoding falsified & dropped:** no exported-globals cap exists (instrumentation scales to 200k+ counters); the apparent 50k-counter slowdown was wasmtime compiling a pathological single-function module, not witness. Inlined-subroutine DWARF + module-rollup: not re-verified this session. | DWARF parsing memory bound at scale. |
| v0.8 | Visualisation tooling (per `v08-visualisation-roadmap.md`). Axum router compiled to `wasm32-wasip2` via `wstd-axum`, `maud` server-side templating, HTMX 2.x, `wasmtime serve` or `wac plug` composed in-process. AI-agent surface as REST+JSON + MCP transport mounted on same router. Playwright tests adopt rivet's pattern. | None blocking. |
| v0.9 | State-of-the-art MC/DC tooling positioning (per `v09-soa-and-agent-ux.md`). MCP server with MC/DC-native tool calls (`find_missing_witness`, `propose_test_row_to_close_gap`, `verify_gap_closed`). Signed agent attribution in the rivet V-model. Complete MC/DC for a high-complex Rust application — destination workload TBD per v0.7 selection. | Competitive: RapiCover already has unbounded conditions for C/Ada. |
