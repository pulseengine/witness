...
| v0.5 | Workspace split: `witness-core` library compiles to wasm32-wasip2; `witness` binary keeps the wasmtime runner. New `witness lcov` subcommand emits LCOV with DWARF-correlated BRDA records and a sibling overview text for uncorrelated branches. New `witness attest` subcommand produces DSSE-signed envelopes via `wsc-attestation` (path-dep on sigil's crate; Ed25519 signing). CI dogfood loop instruments the fixture and uploads bytecode coverage to codecov as `flag: wasm-bytecode` alongside `flag: rust-source`. witness-core Wasm artefact attached to releases. Loom + meld upstream issue drafts captured for v0.6. **Note: v0.5's CHANGELOG described coverage as MC/DC but the reporter computed only branch coverage; v0.6 closes that gap.** |
| v0.6 | **Real MC/DC consumer side.** RunRecord schema v3: `decisions: Vec<DecisionRecord>` with sparse-evaluated rows. New `mcdc_report` module emits truth tables, independent-effect citations under masking MC/DC, gap-closure recommendations. `witness report --format mcdc` / `--format mcdc-json`. Verdict suite of seven canonical compound-decision shapes with hand-derived `TRUTH-TABLE.md` oracles and per-verdict `V-MODEL.md`. Four research agents wrote v0.7-0.9 roadmap docs (httparse scaling, Axum+HTMX visualisation, MCP-server agent UX). The on-Wasm trace-buffer instrumentation lands as v0.6.1 — see `docs/research/v06-instrumentation-primitive.md`. The original v0.6 loom/meld + component-model scope shifts to v0.7+. |
| v0.6.1 | On-Wasm trace-buffer instrumentation per the v0.6 research recommendation. Runner reads per-row trace records and populates `RunRecord.decisions` from execution. Verdict suite end-to-end execution; `compliance` ships real per-verdict signed predicates. |
| v0.7 | Scaling to httparse-class real applications (~1500 decisions). Streaming counter encoding, inlined-subroutine DWARF, BrTable per-row reconstruction. **Reconciled 2026-06:** BrTable per-row shipped v0.33 (FEAT-038, bit-audit; fixed a soundness bug). i64 counters shipped v0.32 (FEAT-036). **Streaming counter encoding falsified** (no globals cap; scales to 200k+ counters — the 50k slowdown was wasmtime compile of a pathological single-fn module). Inlined-subroutine DWARF: not re-verified. |
| v0.8 | Visualisation: Axum+HTMX+wstd-axum component, MCP transport on same router, Playwright-tested per rivet pattern. |
| v0.9 | State-of-the-art positioning: MCP server with MC/DC-native tool calls, signed agent attribution, complete MC/DC for a high-complex Rust application. |
